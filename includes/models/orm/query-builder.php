...
namespace DeliciousBrains\Mergebot\Models\ORM;

/**
 * Progressively build up a query to get results using an easy to understand
 * DSL.
 *
 * The basis of this class is from https://github.com/brandonwamboldt/wp-orm with some tweaks along the way.
 *
...
class Query_Builder {
	 ...
	 *
	 * @param bool $only_count Whether to only return the row count
	 * @param bool $raw Whether to return array of Model objects or raw array
	 *
	 * @return array
	 */
	public function find( $only_count = false, $raw = false ) {
		global $wpdb;
		...
		if ( is_null( $results ) || $raw ) {
			return $results;
		}

		// Turn the items into Models
	...
	/**
	 * Compose search part in the WHERE clause
	 *
	 * @param string $where
	 *
	 * @return string
	 */
	protected function compose_search( $where = '' ) {
	...
	/**
	 * Simple mapping of WHERE clauses
	 *
	 * @return array
	 */
	protected function where_mapping() {
		$mapping = array(
			'where'    => ' AND `%s` = "%s"',
			'not'      => ' AND `%s` != "%s"',
			'like'     => ' AND `%s` LIKE "%s"',
			'not_like' => ' AND `%s` NOT LIKE "%s"',
			'lt'       => ' AND `%s` < "%s"',
			'lte'      => ' AND `%s` <= "%s"',
			'gt'       => ' AND `%s` > "%s"',
			'gte'      => ' AND `%s` >= "%s"',
	 ...
	 * @return string
	 */
	protected function compose_where( $where = '' ) {
		$mapping = $this->where_mapping();

		foreach ( $this->where as $q ) {
			if ( isset( $mapping[ $q['type'] ] ) ) {
				$where .= sprintf( $mapping[ $q['type'] ], $q['column'], esc_sql( $q['value'] ) );
			} elseif ( $q['type'] == 'in' ) {
	 ...
	 * @return string
	 */
	protected function compose_order_by() {
	 ...
	 * @return string
	 */
	protected function compose_limit() {
		$limit = '';
		if ( $this->limit > 0 ) {
	 ...
	 * @return string
	 */
	protected function compose_offset() {
		$offset = '';
		if ( $this->offset > 0 ) {
	...
	public function compose_query( $only_count = false ) {
		...
		// Search
		$where = $this->compose_search();

		$where = $this->compose_where( $where );
		...
		$order = $this->compose_order_by();

		$limit = $this->compose_limit();

		$offset = $this->compose_offset();
